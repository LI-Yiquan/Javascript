...
const fold_left = function (f, base, ls) {
  ...
  return /** <FILL-IN> **/ fold_left(f, f(base, ls.get(0)), ls.shift());/** </FILL-IN> **/
};
...
const map = (g, ls) => /** <FILL-IN> **/ {
  var result = List([]);

  const accumulator = (acc, x) => {
    result = result.push(g(x));
    return acc; 
  };

  fold_left(accumulator, null, ls);
...
const filter = (g, ls) => /** <FILL-IN> **/ {
  var result = List([]);

  const filterCondition = (acc, x) => {
    if (g(x)) {
  ...
  fold_left(filterCondition, null, ls);
...
const partition = (g, ls) => /** <FILL-IN> **/ {
  var satisfyingElements = filter(g, ls);

  var nonSatisfyingElements = filter((x) => !g(x), ls);

  var result = List([satisfyingElements, nonSatisfyingElements]);
...
const quicksort = function(ls) { 
  ...
  /** <FILL-IN> **/
  const pivot = ls.get(0);

  const par_res = partition((x) => x <= pivot, ls.shift());

  const lessOrEqual = par_res.get(0);
  const greater = par_res.get(1);

  var result = quicksort(lessOrEqual).push(pivot);

  result = result.concat(quicksort(greater));
...
assert(quicksort(List([5])).equals(List([5])));
