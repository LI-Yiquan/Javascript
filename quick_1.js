...
const map = (g, ls) => {
  const accumulator = (acc, x) => acc.concat([g(x)]);
  return fold_left(accumulator, List([]), ls);
};
...
const partition = (g, ls) => {
  ...
  return List([List(leftPartition), List(rightPartition)]);
};
...
const quicksort = function(ls) { 
  ...
  /*var result = [...quicksort(lessOrEqual), pivot, ...quicksort(greater)];*/
  
  return result = quicksort(lessOrEqual).push(pivot).concat(quicksort(greater));
